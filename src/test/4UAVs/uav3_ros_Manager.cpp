...
uav3_ros_Manager::uav3_ros_Manager() :
        arm_i_(5),
        is_arm_(false),
        is_offboard_(false),
        is_takeoff_(false),
...
void uav3_ros_Manager::uavOnInit(ros::NodeHandle &nh) {
    state_sub = nh.subscribe<mavros_msgs::State>
            ("mavros/state", 10, &uav3_ros_Manager::state_cb, this);
    vfr_hud_sub = nh.subscribe<mavros_msgs::VFR_HUD>
            ("mavros/vfr_hud", 10,  &uav3_ros_Manager::vrf_hud_cb, this);
    local_position_sub = nh.subscribe<geometry_msgs::PoseStamped>
            ("mavros/local_position/pose", 20,  &uav3_ros_Manager::local_pos_cb, this);
    mavlink_from_sub = nh.subscribe<mavros_msgs::Mavlink>
            ("mavlink/from", 10, &uav3_ros_Manager::mavlink_from_sb, this);
    global_pos_sub = nh.subscribe<sensor_msgs::NavSatFix>
            ("mavros/global_position/global", 10, &uav3_ros_Manager::global_pos_cb, this);
    commander_sub = nh.subscribe<mavros_msgs::DebugValue>
            ("mavros/debug_value/debug_vector", 10, &uav3_ros_Manager::debug_value_cb, this);
    way_point_sub = nh.subscribe<mavros_msgs::WaypointList>
            ("mavros/mission/waypoints", 10, &uav3_ros_Manager::wayPointCB, this);
    homePos_sub = nh.subscribe<mavros_msgs::HomePosition>
            ("mavros/home_position/home", 10, &uav3_ros_Manager::homePositionCB, this);
    ...
    exec_timer_ = nh.createTimer(ros::Duration(0.01), &uav3_ros_Manager::drone_pos_update, this);
    commander_timer_ = nh.createTimer(ros::Duration(0.01), &uav3_ros_Manager::commander_update, this);
    publish_timer_ = nh.createTimer(ros::Duration(0.01), &uav3_ros_Manager::publishDronePosControl, this);
}

void uav3_ros_Manager::state_cb(const mavros_msgs::State::ConstPtr& msg) {
    uav_.current_state = *msg;
}

void uav3_ros_Manager::vrf_hud_cb(const mavros_msgs::VFR_HUD::ConstPtr &msg) {
    current_vfr_hud = *msg;
}

void uav3_ros_Manager::local_pos_cb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
    ...
    chlog::info("data","uav3 m_heading = %.2f", dronepos_.m_heading);
}

void uav3_ros_Manager::mavlink_from_sb(const mavros_msgs::Mavlink::ConstPtr& msg) {
    current_mavlink = *msg;
    uav_.drone_id = current_mavlink.sysid;
//    chlog::info("data","sys_id = %d", current_mavlink.sysid);
}

void uav3_ros_Manager::global_pos_cb(const sensor_msgs::NavSatFix::ConstPtr& msg) {
    uav_.altitude = msg->altitude;
    uav_.longtitude = msg->longitude;
    uav_.latitude = msg->latitude;
}

void uav3_ros_Manager::debug_value_cb(const mavros_msgs::DebugValue::ConstPtr& msg) {
    mavros_msgs::DebugValue debugValue;
    debugValue = *msg;
    chlog::info("data","uav3 debug_value x = %.2f, y = %.2f, z = %.2f", debugValue.data[0], debugValue.data[1],
             debugValue.data[2]);
...
void uav3_ros_Manager::commander_update(const ros::TimerEvent& e) {
    ...
    if (command == VF_UAV_ALL_START /*|| command == SLAVESTART*/) {
        ...
        chlog::info("data","uav3 arm_i = %d, is_arm = %d", arm_i_, is_arm_);
        if (!current_state.armed && !is_arm_) {
            while(arm_i_ > 0) {
                if (arming_client.call(arm_cmd) &&
                    arm_cmd.response.success) {
                    chlog::info("data","uav3 Vehicle armed");
                    is_arm_ = true;
                }
                --arm_i_;
            }
            ...
            for (takeoff_i = 10; ros::ok() && takeoff_i > 0; --takeoff_i) {
                if (current_state.mode != "AUTO.TAKEOFF") {
                    if (set_mode_client.call(takeoff_set_mode)  &&
                        ...
                        chlog::info("data","uav3 Takeoff enabled");
                        is_takeoff_ = true;
                    }
                }
            }
        ...
        if (current_state.mode != "OFFBOARD" && !is_offboard_) {
            ...
            for (offboard_i = 10; ros::ok() && offboard_i > 0; --offboard_i) {
                if (set_mode_client.call(offb_set_mode) &&
                    ...
                    chlog::info("data","uav3 Offboard enabled");
                    is_offboard_ = true;
                }
            }
        }
        ...
        if (current_state.mode != "AUTO.Land" && !is_land_) {
            ...
            for (land_i = 10; ros::ok() && land_i > 0; --land_i) {
                if (set_mode_client.call(land_set_mode) &&
                    ...
                    chlog::info("data","uav3 Return enabled");
                    is_land_ = true;
                }
            }
        }
...
void uav3_ros_Manager::drone_pos_update(const ros::TimerEvent& e) {
...
void uav3_ros_Manager::publishDronePosControl(const ros::TimerEvent& e) {
    if (is_speed_ctrl_) {
        drone_yaw_control();
...
void uav3_ros_Manager::drone_yaw_control() {
    ...
    chlog::info("data","yaw_rate = %.2f, target_heading_ = %.2f, dronepos_.m_heading = %.2f", yaw_rate, target_heading_,
             dronepos_.m_heading);
}
...
void uav3_ros_Manager::wayPointCB(const mavros_msgs::WaypointList::ConstPtr &msg) {
    uav_.waypointList = *msg;
    chlog::info("data","uav3 mission waypoint size = %d", uav_.waypointList.waypoints.size());
}

void uav3_ros_Manager::homePositionCB(const mavros_msgs::HomePosition::ConstPtr& msg){
    uav_.homePosition = *msg;
    chlog::info("data","uav4 home position lat = %.8f, lon = %.8f", msg->geo.latitude, msg->geo.longitude);
}