...
void MultiOffboard::usv_targte_local_pos() {
    ...
    if (drone_uav5_.current_state.mode == "OFFBOARD") {
        switch (usv_state_) {
            case USA_INIT:
            ...
            case USA_WAYPOINT:
                if (!usv_way_points.empty()) {
                    drone_uav5_.target_pose = usv_way_points.back();
    ...
    drone_uav6_.target_pose.pose.position = drone_uav5_.target_pose.pose.position;
    drone_uav7_.target_pose.pose.position = drone_uav5_.target_pose.pose.position;


    if (uav_state_ == FALLOW_USV) {
        drone_uav5_.local_pos_pub.publish(drone_uav5_.target_pose);
        drone_uav6_.local_pos_pub.publish(drone_uav6_.target_pose);
        drone_uav7_.local_pos_pub.publish(drone_uav7_.target_pose);
    }
}

void MultiOffboard::uav_targte_local_pos() {
    if (drone_uav2_.current_state.mode == "OFFBOARD") {
        switch (uav_state_) {
            // takeoff
            case TAKEOFF:
                ...
                if (pos_reached(drone_uav2_.current_local_pos, target_pos_, 0.8)){
            ...
            case FALLOW_USV:
                if (drone_uav5_.current_state.armed) {
                    target_pos_.pose.position.x = drone_uav5_.target_pose.pose.position.x;
                    target_pos_.pose.position.y = drone_uav5_.target_pose.pose.position.y;
                }
                break;
        ...
        drone_uav2_.target_pose = target_pos_;
    } else {
        drone_uav2_.target_pose.pose.position = uav2_current_local_pos_sp.position;
        mavros_msgs::SetMode land_set_mode;
//        mavros_msgs::SetMode return_set_mode;
        land_set_mode.request.custom_mode = "AUTO.LAND";

        if ((drone_uav1_.current_state.mode != "AUTO.LAND" || drone_uav3_.current_state.mode != "AUTO.LAND" ||
             drone_uav4_.current_state.mode != "AUTO.LAND") && drone_uav2_.current_state.mode == "AUTO.LAND" && is_armed) {
            if( drone_uav1_.set_mode_client.call(land_set_mode) &&
                land_set_mode.response.mode_sent){
                drone_uav2_.set_mode_client.call(land_set_mode);
                drone_uav3_.set_mode_client.call(land_set_mode);
    ...
    drone_uav1_.target_pose.pose.position = drone_uav2_.target_pose.pose.position;
    drone_uav3_.target_pose.pose.position = drone_uav2_.target_pose.pose.position;
    drone_uav4_.target_pose.pose.position = drone_uav2_.target_pose.pose.position;
